...
class DownloadManager:
    ...
    def _get_cookie_args(self) -> List[str]:
        ...
        v18.1.4: Added browser cookie support to help bypass 403 errors.
        Using cookies from an authenticated browser session can help
        with YouTube's anti-bot measures for high-resolution downloads.
        """
        ...
        if self.settings_mgr.get("use_browser_cookies", False):
            browser = self.settings_mgr.get("cookies_browser", "safari")
    ...
    def _download_task(self, task: DownloadTask):
        ...
        try:
            ...
            if fmt and fmt.height:
                if fmt.height >= 2160:
                    # For 4K: Use the exact format ID if available (most reliable)
                    # Otherwise let yt-dlp select the best 4K format
                    # v18.1.4: After first failures, fall back to generic format selection
                    # which may find alternative 4K formats that aren't blocked
                    if fmt.format_id and fmt.format_id not in ("", "unknown"):
                        video_format = fmt.format_id
                        # Store the fallback format in case we need to retry with it
                        ...
                        # Let yt-dlp pick the best video, prioritizing resolution
            ...
            ] + cookie_args + video_cmd_args_extra + [
                "-f", video_format,
                "-o", temp_video,
                video_info.url
            ]
            ...
            for attempt in range(RETRY_MAX_ATTEMPTS):
                ...
                if attempt > 0:
                    ...
                    # v18.1.4: After 3 failures with specific format ID, try generic format selection
                    # YouTube may be blocking specific format IDs but allowing others
                    if attempt >= 3 and hasattr(self, '_4k_fallback_format') and self._4k_fallback_format and not tried_fallback:
                        tried_fallback = True
                        self._notify("log", ("info", "Trying alternative 4K format selection..."))
                        fallback_cmd_args = [
                            "--newline",
                ...
                if attempt < RETRY_MAX_ATTEMPTS - 1:
                    ...
                    if attempt >= RETRY_SILENT_THRESHOLD:
                        ...
                        if not self.settings_mgr.get("use_browser_cookies", False):
                            self._notify("log", ("info", "💡 Tip: Enable browser cookies in Settings → Advanced for better success"))
            ...
            if task.status == DownloadStatus.FAILED:
                self._notify("log", ("error", f"❌ Download failed after {RETRY_MAX_ATTEMPTS} attempts"))
...
class SettingsManager:
    ...
    def _load_settings(self) -> Dict[str, Any]:
        ...
        defaults = {
            ...
            # Browser Cookies (v18.1.4) - helps bypass 403 errors
            "use_browser_cookies": False,
            "cookies_browser": "safari",  # safari, firefox, chrome
        }
...
class SettingsWindow(ctk.CTkToplevel):
    ...
    def _create_advanced_tab(self):
        """Advanced settings for 403 bypass and other workarounds."""
        ...
        # Info banner about YouTube blocking
        info_frame = ctk.CTkFrame(scroll_frame, fg_color="#4d1c1c", corner_radius=8)
        info_frame.pack(fill="x", padx=10, pady=(10, 15))
        
        ctk.CTkLabel(
            info_frame,
            text="🚫 Downloads Being Blocked?",
            font=ctk.CTkFont(size=14, weight="bold"),
            text_color="#ff9999"
        ).pack(anchor="w", padx=15, pady=(10, 5))
        ...
        ctk.CTkLabel(
            info_frame,
            text="YouTube actively blocks video downloads. If you see 'YouTube blocked'\nor '403 Forbidden' errors, enable browser cookies below.\n\nThis uses your YouTube login to authenticate downloads,\nwhich significantly improves success rates.",
            font=ctk.CTkFont(size=12),
            text_color="#cccccc",
            justify="left"
        ).pack(anchor="w", padx=15, pady=(0, 10))
        ...
        ctk.CTkLabel(
            scroll_frame,
            text="Browser Cookies (Recommended)",
            font=ctk.CTkFont(size=16, weight="bold")
        ).pack(anchor="w", padx=10, pady=(15, 10))
        
        self.use_cookies = ctk.CTkCheckBox(
            scroll_frame, 
            text="Use cookies from browser (RECOMMENDED for reliable downloads)",
            font=ctk.CTkFont(size=13)
        )
        self.use_cookies.pack(anchor="w", padx=20, pady=(5, 10))
        if self.settings_mgr.get("use_browser_cookies", False):
            self.use_cookies.select()
        
        # Browser selection
        browser_frame = ctk.CTkFrame(scroll_frame, fg_color="transparent")
        browser_frame.pack(anchor="w", padx=20, pady=(0, 10))
        ...
        self.cookies_browser.pack(side="left")
        self.cookies_browser.set(self.settings_mgr.get("cookies_browser", "safari"))
        
        # Recommendation note
        note_frame = ctk.CTkFrame(scroll_frame, fg_color="#1c3d1c", corner_radius=8)
        note_frame.pack(fill="x", padx=10, pady=(10, 15))
        ...
        ctk.CTkLabel(
            note_frame,
            text="1. Log into YouTube in Safari or Firefox\n"
                 "2. Check the box above and select your browser\n"
                 "3. Close the browser completely\n"
                 "4. Start your download\n\n"
                 "💡 Safari or Firefox work best on macOS.\n"
                 "   Chrome may have cookie database locking issues.",
            font=ctk.CTkFont(size=12),
            text_color="#aaaaaa",
            justify="left"
        ).pack(anchor="w", padx=15, pady=(0, 10))
    ...
    def _save(self):
        ...
        self.settings_mgr.set("cookies_browser", self.cookies_browser.get())
...
"403 Forbidden" or download failures:
  YouTube frequently updates their systems. Click the Update button
  (🔄) and select "Nightly Build" to get the latest fixes.
  Nightly builds are updated daily with YouTube workarounds.
